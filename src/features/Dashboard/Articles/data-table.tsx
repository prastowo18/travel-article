...
export function DataTable() {
  ...
  const [action, setAction] = React.useState<'add' | 'update'>('add');
  ...
  const articlesQuery = useArticle({
    page: String(pagination.pageIndex + 1),
    pageSize: String(pagination.pageSize),
  });
  const { data: articlesData, isLoading: isLoadingArticles } = articlesQuery;

  const { mutate: mutateDelete, isPending: isPendingDelete } =
    useDeleteArticle();

  React.useEffect(() => {
    if (!isLoadingArticles && articlesData?.data) {
  ...
  const handleDelete = (article: ArticleType) => {
    mutateDelete(article.documentId);
  };
    ...
    >
      <div className="flex items-center justify-end px-4 lg:px-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button variant="outline" size="sm" onClick={onAddCategories}>
            <PlusIcon />
      ...
      >
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center justify-end w-full gap-2 p-5 ml-auto lg:ml-0 lg:w-fit">
            <div className="flex items-center justify-center text-sm font-medium w-fit">
              Page{' '}
              {isLoadingArticles
                ? 0
                : table.getState().pagination.pageIndex + 1}{' '}
              of {isLoadingArticles ? 0 : table.getPageCount()}
            </div>
            <Button
              variant="outline"
